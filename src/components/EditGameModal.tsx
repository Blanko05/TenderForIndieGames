...
export function EditGameModal({ game, onClose, onSubmit }: EditGameModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!validateItchUrl(formData.itch_url)) {
      setError('Please enter a valid itch.io URL');
      return;
    }
    ...
    <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
      <div className="bg-slate-800 rounded-lg shadow-xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-slate-700">
          <h2 className="text-2xl font-bold text-white">Edit Game Profile</h2>
          <button
            onClick={onClose}
