...
export function CreateReelModal({ onClose, onSubmit }: CreateReelModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    console.log('Submit clicked. Upload method:', uploadMethod);
    ...
    try {
      ...
      if (uploadMethod === 'file' && videoFile) {
        ...
        console.log('File uploaded, video URL:', videoUrl);
      }

      console.log('Submitting reel with video URL:', videoUrl);
      await onSubmit({
        video_url: videoUrl,
      ...
      setError(`Failed to create reel: ${err instanceof Error ? err.message : 'Unknown error'}`);
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
      <div className="bg-slate-800 rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-slate-700">
          <h2 className="text-2xl font-bold text-white">Create Reel</h2>
          <button
            onClick={onClose}
          ...
          <div>
            ...
            {uploadMethod === 'file' ? (
              <div>
                ...
                >
                  <div className={`w-full px-4 py-8 bg-slate-900 border-2 border-dashed rounded-lg text-center transition-colors ${
                    videoFile ? 'border-green-500 bg-green-900/20' : 'border-slate-600 hover:border-blue-500'
                  }`}>
                    <Upload className={`w-8 h-8 mx-auto mb-2 ${videoFile ? 'text-green-400' : 'text-slate-400'}`} />
